...
import { Context, BackfillScopeCb } from '../../../src/context/context'
...
describe('Context', function () {
  ...
  // })
  //
  describe('BackfillScopeCb', function () {
    it('allows populating scope when backfillScopeCb is called', function () {
      const backfillScopeCb: BackfillScopeCb = async function (variable, envVar) {
        const prom = new Promise(
          resolve => setTimeout(() => resolve('zoo'), 10)
        )
      ...
      const ctx = new Context({ }, defaultOptions, false, backfillScopeCb)
      const res = ctx.get(['foo'])
